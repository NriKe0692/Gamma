 ...
 * @author Gustavo
 */
public class RegistrarTransaccionSalida extends HttpServlet {
...
//            String stock = jsonObj.getString("stock");
            String invsalida = jsonObj.getString("inventariosalida");
            ...
            String destino = jsonObj.getString("destinosalida");

            Date fecha = new Date();
              SimpleDateFormat formatofecha = new SimpleDateFormat("dd-MM-YY");
              Date fechafac=formatofecha.parse(fechafactura);
            transacciondao.RegistrarTransaccion(fecha, nrofactura,fechafac, observacion, serie, Integer.parseInt(invsalida), 100000, codigo, destino, 2);
            
            JSONObject json = new JSONObject(); 

            json.put("fechasalida", formatofecha.format(fecha));
            json.put("fecha",fechafactura );
            json.put("tipo", "Salida");
            json.put("serieproducto", serie);
            json.put("proveedor", "Empresa");
            json.put("destino", destino);
            json.put("nrofactura", nrofactura);
            json.put("fechafactura", fechafactura);
            json.put("cantidadsalida", invsalida);
